...
--
	...
	local function prepare()
		...
		cfg = genie.solution.getproject(sln, 1)
	end
...
--
	function suite.IsDebug_ReturnsFalse_EnglishSpellingOfOptimiseFlag()
		flags { "Optimise" }
		prepare()
		return test.isfalse(genie.config.isdebugbuild(cfg))
	end
	...
	function suite.IsDebug_ReturnsTrue_OnSymbolsFlag()
		flags { "Symbols" }
		prepare()
		return test.istrue(genie.config.isdebugbuild(cfg))
	end

	function suite.shouldIncrementallyLink_staticLib_returnsFalse()
		kind "StaticLib"
		prepare()
		return test.isfalse(genie.config.isincrementallink(cfg))
	end
	...
	function suite.shouldIncrementallyLink_notStaticLib_NoIncrementalLinkFlag_noOptimiseFlag_returnsTrue()
		prepare()
		return test.istrue(genie.config.isincrementallink(cfg))
	end
