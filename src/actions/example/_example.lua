...
-- you can remove any of the additional fields that are not required by your action.
	...
	newaction 
	{
		...
		-- this action this run (genie example)
		trigger = "example",
		...
		-- The description is shown in the help text (genie /help)
		...
		-- genie.generate() creates a file for you in the correct place, taking into
		...
		onsolution = function(sln)
			genie.generate(sln, "%%.sln.txt", genie.example.solution)
		end,

		onproject = function(prj)
			if genie.isdotnetproject(prj) then
				genie.generate(prj, "%%.csprj.txt", genie.example.project)
			else
				genie.generate(prj, "%%.cprj.txt", genie.example.project)
			end
		end,
		...
		-- genie.clean.file() and genie.clean.directory() are convenience functions
		-- that use the same pattern matching as genie.generate() above.

		oncleansolution = function(sln)
			genie.clean.file(sln, "%%.sln.txt")
		end,
		
		oncleanproject  = function(prj)
			if genie.isdotnetproject(prj) then
				genie.clean.file(prj, "%%.csprj.txt")
			else
				genie.clean.file(prj, "%%.cprj.txt")
			end
		end,
