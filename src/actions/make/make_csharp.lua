...
--
	...
	function genie.make_csharp(prj)
		local csc = genie.dotnet
		...
		for cfg in genie.eachconfig(prj) do
			anycfg = cfg
			cfglibs[cfg] = genie.getlinks(cfg, "siblings", "fullpath")
			cfgpairs[cfg] = { }
		...
		for fcfg in genie.project.eachfile(prj) do
		...
		for _, libname in ipairs(genie.getlinks(prj, "system", "fullpath")) do
		...
		local platforms = genie.filterplatforms(prj.solution, genie[_OPTIONS.cc].platforms)
		table.insert(platforms, 1, "")

		-- write the configuration blocks
		for cfg in genie.eachconfig(prj) do
			genie.gmake_cs_config(cfg, csc, cfglibs)
		end
		...
		_p('REFERENCES += %s', table.implode(_MAKE.esc(genie.getlinks(prj, "system", "basename")), "/r:", ".dll", " "))
		_p('')
		...
		for cfg in genie.eachconfig(prj) do
			...
			for target, source in pairs(cfgpairs[cfg]) do
				genie.make_copyrule(source, target)
			end
			_p('endif')
			_p('')
		end
...
--
	...
	function genie.gmake_cs_config(cfg, csc, cfglibs)
		...
		_p('  DEPENDS    := %s', table.concat(_MAKE.esc(genie.getlinks(cfg, "dependencies", "fullpath")), " "))
