...
--
	...
	genie.make.cpp = { }
	genie.make.override = { }
	genie.make.makefile_ignore = false

	local cpp = genie.make.cpp
	local make = genie.make

	function genie.make_cpp(prj)
		...
		genie.gmake_cpp_header(prj, cc, platforms)
		genie.gmake_cpp_configs(prj, cc, platforms)
		...
		_p('$(TARGETDIR):')
		genie.make_mkdirrule("$(TARGETDIR)")
...
--
	...
	function genie.gmake_cpp_header(prj, cc, platforms)
		_p('# %s project makefile autogenerated by GENie', genie.action.current().shortname)

		-- set up the environment
		_p('ifndef config')
		_p('  config=%s', _MAKE.esc(genie.getconfigname(prj.solution.configurations[1], platforms[1], true)))
		_p('endif')
		_p('')
		...
		if (not genie.make.makefile_ignore) then
...
--
	...
	function genie.gmake_cpp_configs(prj, cc, platforms)
		for _, platform in ipairs(platforms) do
			for cfg in genie.eachconfig(prj, platform) do
				genie.gmake_cpp_config(prj, cfg, cc)
			end
		end
	end

	function genie.gmake_cpp_config(prj, cfg, cc)
		...
		_p('  ' .. (table.contains(genie.make.override,"OBJDIR") and "override " or "") ..    'OBJDIR              = %s', _MAKE.esc(cfg.objectsdir))
		_p('  ' .. (table.contains(genie.make.override,"TARGETDIR") and "override " or "") .. 'TARGETDIR           = %s', iif(targetDir == "", ".", targetDir))
		_p('  ' .. (table.contains(genie.make.override,"TARGET") and "override " or "") ..    'TARGET              = $(TARGETDIR)/%s', _MAKE.esc(cfg.buildtarget.name))
...
--
	...
	function cpp.linker(prj, cfg, cc)
		...
		if #cfg.wholearchive > 0 then
			...
			for _, linkcfg in ipairs(genie.getlinks(cfg, "siblings", "object")) do
			...
			libdeps = make.list(_MAKE.esc(genie.getlinks(cfg, "siblings", "fullpath")))
			lddeps  = libdeps
		end
