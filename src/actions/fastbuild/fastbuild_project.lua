...
local function compile(indentlevel, prj, cfg, commonbasepath)
	...
	if cfg.flags.Symbols then
		if (cfg.flags.C7DebugInfo) then
			...
			if genie.config.iseditandcontinue(cfg) then
...
local function binary(prj, cfg, useconfig, bintype, commonbasepath)
	...
	local sorteddeplibs = {}
	for _, deplib in ipairs(genie.getlinks(cfg, "dependencies", "basename")) do
	...
	table.insert(linkeroptions, '/INCREMENTAL'..((genie.config.isincrementallink(cfg) and '') or ':NO'))

	for _, addloption in ipairs(cfg.linkoptions) do
		table.insert(linkeroptions, addloption)
	end

	local linklibs = genie.getlinks(cfg, "all", "fullpath")
...
function genie.fastbuild.project(prj)
	io.indent = '    '
	...
	local cppfiles = {}
	for file in genie.project.eachfile(prj) do
