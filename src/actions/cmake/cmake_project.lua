...
function cmake.header(prj)
    _p('# %s project autogenerated by GENie', genie.action.current().shortname)
    _p('cmake_minimum_required(VERSION 2.8.4)')
    _p('')
    _p('project(%s)', genie.esc(prj.name))
end
    ...
    for _, dependency in ipairs(prj.dependency) do
        for _, dep in ipairs(dependency) do
            if path.issourcefile(dep[1]) then
                local dep1 = genie.esc(path.getrelative(prj.location, dep[1]))
                local dep2 = genie.esc(path.getrelative(prj.location, dep[2]))
...
function cmake.removeCrosscompiler(platforms)
    for i = #platforms, 1, -1 do
        if genie.platforms[platforms[i]].iscrosscompiler then
...
function cmake.project(prj)
    ...
    for _, cfg in ipairs(configurations) do
        ...
        if (prj.kind == 'StaticLib') then
            _p(1, 'add_library(%s STATIC ${source_list})', genie.esc(cfg.buildtarget.basename))
        end

        if (prj.kind == 'SharedLib') then
            _p(1, 'add_library(%s SHARED ${source_list})', genie.esc(cfg.buildtarget.basename))
        end
        if (prj.kind == 'ConsoleApp' or prj.kind == 'WindowedApp') then
            _p(1, 'add_executable(%s ${source_list})', genie.esc(cfg.buildtarget.basename))
            _p(1, 'target_link_libraries(%s%s%s)', genie.esc(cfg.buildtarget.basename), cmake.list(genie.esc(genie.getlinks(cfg, "siblings", "basename"))), cmake.list(cc.getlinkflags(cfg)))
        end
        _p('endif()')
        _p('')
    end
